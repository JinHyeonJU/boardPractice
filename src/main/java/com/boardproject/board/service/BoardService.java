...
public class BoardService {
    ...
    public BoardDTO update(BoardDTO boardDTO) {
        ...
        boardRepository.save(boardEntity); //id값이 있냐 없냐에 업데이트할지 생성할지
    ...
    public Page<BoardDTO> paging(Pageable pageable) {
        int page = pageable.getPageNumber() - 1; // -1 하는이유는 page 위치에 있는 값은 0부터 시작. 실제는 1페이지지만 값은 0
        ...
        Page<BoardDTO> boardDTOS = boardEntities.map(board -> new BoardDTO(board.getId(), board.getBoardWrite(), board.getBoardTitle(), board.getBoardHits(), board.getCreatedTime()));
        return boardDTOS;
    }
}
